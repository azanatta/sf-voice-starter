...
/** `sf org display` — obtains the org identity and API session used by CLI phases. */
...
interface OrgOpenUrl {
  url: string;
}

/** Gets the short-lived one-time browser-login URL generated by Salesforce CLI. */
export function orgOpenUrl(orgAlias: string, options?: SfRunOptions): Promise<OrgOpenUrl> {
  return sf<OrgOpenUrl>(['org', 'open', '--target-org', orgAlias, '--url-only'], options);
}
